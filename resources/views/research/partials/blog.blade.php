<div class="panel panel-primary">
    <div class="panel-heading">
        <span class="glyphicon glyphicon-file"></span> Editais
    </div>
    @if(count($editals))
        <div class="list-group">
            @foreach($editals as $edital)
                <a class="list-group-item" href="{{ route('blog.edital', $edital->id) }}">
                    <h5 class="list-group-item-heading">{{ $edital->title }}</h5>
                    <small class="list-group-item-text">{{ $edital->started_at->format('d/m/Y') }} - {{ $edital->finished_at->format('d/m/Y') }}</small>
                </a>
            @endforeach
...
<div class="panel panel-primary">
    <div class="panel-heading">
        <span class="glyphicon glyphicon-bullhorn"></span> Notícias
    </div>
    @if(count($newses))
        <div class="list-group">
            @foreach($newses as $news)
                <a class="list-group-item" href="{{ route('blog.news', $news->id) }}">
                    <h5 class="list-group-item-heading">{{ $news->title }}</h5>
                    <small class="list-group-item-text">Publicado em {{ $news->publicated_at->format('d/m/Y') }}</small>
                </a>
            @endforeach
        ...
        <div class="list-group">
            @foreach($docs as $doc)
                <a class="list-group-item" href="{{ route('blog.document', $doc->id) }}">
                    <h5 class="list-group-item-heading">{{ $doc->title }}</h5>
                    <small class="list-group-item-text">Publicado em {{ $doc->created_at->format('d/m/Y') }} </small>
                </a>
            @endforeach
